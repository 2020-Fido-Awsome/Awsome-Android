...
class AuthnActivity : AppCompatActivity() {
    ...
    fun registerFido2() {
        ...
        val options = PublicKeyCredentialCreationOptions.Builder()
            .setRp(PublicKeyCredentialRpEntity("awsomewifi.firebaseapp.com", "awsome2020", null))
            .setUser(
            ...
            .build()
        println("******** test *************")
        println("${responseData.publicKeyCredentialCreationOptions.user.id.toByteArray()}")
        println(responseData.publicKeyCredentialCreationOptions.user.name)
        println(responseData.publicKeyCredentialCreationOptions.user.displayName)
...
//        attestationObjectBase64.replace("\","")
    ...
    private fun storeKeyHandle(keyHandle: ByteArray) {
        DataManage.key_handle = Base64.encodeToString(keyHandle, Base64.DEFAULT)
    }
